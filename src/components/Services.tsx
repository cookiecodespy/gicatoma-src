      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {services.map((service) => {
            ...
            const isDigitalizacion = service.id === 'digitalizacion-documental';

            return (
              <motion.div
                key={service.id}
                className={['relative overflow-hidden rounded-xl shadow-lg p-8 transition-transform duration-200 group cursor-pointer', isDigitalizacion ? 'bg-blue-100 shadow-md' : 'bg-gradient-to-br from-blue-50 via-white to-blue-100/70'].join(' ')}
              ...
              >
                ...
                {/* Icon */}
                <div
                  className={`relative inline-flex p-4 rounded-xl ${isDigitalizacion ? 'bg-blue-500' : 'bg-gradient-to-r ' + service.color} mb-6 shadow-md`}
                >
                  <IconComponent className={isDigitalizacion ? 'h-9 w-9 text-white' : 'h-8 w-8 text-white'} />
                </div>
